...
# Prevents Git Bash from converting paths like
# /health, /ecs/rag-api and /_stcore/health
...
export GROQ_API_KEY="your-groqai-key"
...
```bash
mkdir -p .aws/policies

cat > .aws/policies/ecs-trust-policy.json <<'EOF'
{
  "Version": "2012-10-17",
  "Statement": [{
    "Effect": "Allow",
...
```bash
cat > .aws/policies/rag-secrets-policy.json <<EOF
{
  "Version": "2012-10-17",
  "Statement": [{
    "Effect": "Allow",
...
export SECRETS_POLICY_ARN=$(aws iam create-policy \
  ...
  --policy-document file://.aws/policies/rag-secrets-policy.json \
  --query 'Policy.Arn' \
  --output text)
...
mkdir -p .aws/rendered
render .aws/task-definitions/rag-api.json    .aws/rendered/rag-api.json
render .aws/task-definitions/rag-ui.json     .aws/rendered/rag-ui.json

export RAG_API_TASK_DEF_ARN=$(aws ecs register-task-definition \
  --cli-input-json file://.aws/rendered/rag-api.json \
  --query 'taskDefinition.taskDefinitionArn' \
  --output text)

export RAG_UI_TASK_DEF_ARN=$(aws ecs register-task-definition \
  --cli-input-json file://.aws/rendered/rag-ui.json \
  --query 'taskDefinition.taskDefinitionArn' \
  --output text)
...
aws ecs create-service \
  ...
  --load-balancers "targetGroupArn=$API_TG_ARN,containerName=rag-api,containerPort=8080" \
...
aws ecs create-service \
  ...
  --load-balancers "targetGroupArn=$UI_TG_ARN,containerName=rag-ui,containerPort=8501" \
  --health-check-grace-period-seconds 60 \
  --deployment-configuration "minimumHealthyPercent=100,maximumPercent=200"
```
...
for NAME in neon-db-url upstash-redis-rest-url upstash-redis-rest-token qdrant-url qdrant-api-key groqai-api-key jina-api-key portkey-api-key rag-api-key logfire-token langsmith-api-key; do
...
aws iam detach-role-policy --role-name ecsTaskExecutionRole --policy-arn $SECRETS_POLICY_ARN
